# Parker Aeroponic Life Support System — Touch HMI Feasibility Assessment

**Document version:** 2.1 (Parker confirmed sensors + ADC semantics)  
**Date:** 2026-09-11  
...
**Recommendation:** Proceed. Remaining P0: UART logic level, panel power, enclosure, and a ~1 Hz telemetry heartbeat (moisture itself only updates every ~30 s).
...
Provide a **local touch-screen control panel** for a pump-based plant life-support system:
...
| `temperature_c` | float | optional | Measured temperature if available |
| `pump` | int | optional, 0 / 1 | Actual pump state, so the Switch can resync |
...
|---|---------------|-------|----------|
| 1 | UART logic level: 3.3 V vs 5 V (level shifter if 5 V) | Parker | **P0** |
...
| 4 | Panel power: independent 5 V / 2 A vs Parker-supplied | Both | **P0** |
| 5 | Enclosure against mist/condensation (capacitive touch false triggers) | Both | **P0** |
| 6 | **Heartbeat:** moisture samples every 30 s — can Parker still send telemetry at ~1 Hz? (else timeout ≥ 60 s) | Parker | **P0** |
| 7 | Whether to show resistive air humidity on the HMI | Both | P1 |
| 8 | Probe polarity already “higher = wetter” to match 1844/2457? | Parker | P1 |
| 9 | ACK on commands (recommended) | Both | P1 |
| 10 | Show measured temperature in addition to setpoint | Both | P2 |
...
3. Implement against **`docs/INTEGRATION_GUIDE_en.md`** (the working contract). Freeze protocol v1 (`moisture_*` + 1 Hz heartbeat), then develop in parallel.  
4. Remaining P0: UART level, panel power, enclosure, heartbeat — Parker please reply to the checklist in the integration guide §15.
...
| 2.2 | 2026-09-11 | Parker | All Priority0 dependancies have been met, Priority1 dependancies have been met(pending viewership), and Priority2 dependancies have been met but can be cancelled if workload becomes too-much.
